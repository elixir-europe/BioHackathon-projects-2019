...
def print_publications_with_update(tx, doi, limit=25):
    ...
    return tx.run("MATCH (n1)-[r1]-(n2)-[r2]-(n3) WHERE n1.doi IN " + negatives + " AND n3.doi IN " + negatives + " WITH collect(distinct(n2.doi)) + collect(distinct(n3.doi)) as removeList MATCH (n0)-[r]-(n) WHERE n0.doi IN " + positives + " AND (NOT n.doi IN removeList) AND (NOT n.doi IN " + unvoted + " ) AND (NOT n.doi IN " + positives + " ) RETURN DISTINCT(n) as n2 LIMIT " + str(limit)
    )
